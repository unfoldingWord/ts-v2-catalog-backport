...
from ts_v2_catalog_backport.libraries.tools.file_utils import load_json_object, read_file
from ts_v2_catalog_backport.libraries.tools.mocks import MockS3Handler, MockAPI, MockDynamodbHandler, MockLogger
from ts_v2_catalog_backport.ts_v2_catalog_backport import TsV2CatalogBackporter
from ts_v2_catalog_backport.libraries.tools.test_utils import assert_s3_equals_api_json, assert_json_files_equal
from ts_v2_catalog_backport.libraries.tools.ts_v2_utils import build_usx, index_chunks, convert_rc_links
import tempfile


# This is here to test importing main

@patch('ts_v2_catalog_backport.ts_v2_catalog_backport.ErrorReporter')
class TestTsV2CatalogBackporter(TestCase):

    resources_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'resources')

    def setUp(self):
        self.latest_catalog = load_json_object(os.path.join(TestTsV2CatalogBackporter.resources_dir, 'v3_api/v3', "catalog.json"))
...
"""
    ...
    def test_inprogress(self, mock_reporter):
        ...
        mockDb._load_db(os.path.join(TestTsV2CatalogBackporter.resources_dir, 'ready_inprogress_db.json'))

        mockLog = MockLogger()
        converter = TsV2CatalogBackporter(prefix='',
    ...
    def test_convert_catalog(self, mock_reporter):
        ...
        mockS3 = MockS3Handler('ts_bucket')
        mockDb = MockDynamodbHandler()
        mockDb._load_db(os.path.join(TestTsV2CatalogBackporter.resources_dir, 'ready_new_db.json'))

        mockLog = MockLogger()
        converter = TsV2CatalogBackporter(prefix='test-',
    ...
    def test_complete_status(self, mock_reporter):
        ...
        mockS3 = MockS3Handler('ts_bucket')
        mockDb = MockDynamodbHandler()
        mockDb._load_db(os.path.join(TestTsV2CatalogBackporter.resources_dir, 'complete_db.json'))
        mockLog = MockLogger()
    ...
    def test_missing_catalog(self, mock_reporter):
        ...
        converter = TsV2CatalogBackporter(
                                       prefix='',
                                       cdn_bucket='cdn.door43.org',
                                       cdn_url='https://cdn.door43.org',
                                       logger=mockLog,
    ...
    def test_broken_catalog(self, mock_reporter):
        ...
        result = converter.run()
        self.assertFalse(result)
q        self.assertIn('Failed to load the catalog json: No JSON object could be decoded', mockLog._messages)
    ...
    #     """
    #     This will run through a full cata pip install pytestlog build using live data, though nothing will be uploaded to the server.
    #     :return:
    #     """
    #     mockS3 = MockS3Handler('ts_bucket')
    #     mockLogger = MockLogger()
    #     mockDb = MockDynamodbHandler()
    #     mockDb._load_db(os.path.join(TestTsV2CatalogBackporter.resources_dir, 'ready_new_db.json')) # you can also play with ready_processing_db.json
    #
    #     vars = {
    #         'stage-variables': {
    #             'cdn_bucket': 'cdn.door43.org',
