...
from libraries.tools.lambda_utils import set_lambda_running, lambda_sec_remaining, wipe_temp
...
class TsV2CatalogHandler():
    ...
    def __init__(self, event, context, **kwargs):
        ...
        if 'dynamodb_handler' in kwargs:
            ...
            self.logger.info('{}d43-catalog-status'.format(self.stage_prefix()))
    ...
    def _has_resource_changed(self, pid, lid, rid, modified_at):
        ...
        if cache_key in self.ts_resource_cache:
            self.logger.debug(f"HAVE CACHE-KEY: {cache_key}")
...
def handle(event, context):
    ...
    catalog = TsV2CatalogHandler(event, context)
    return catalog.run()

if __name__ == '__main__':
    # res = index_obs("shu", "obs", {
    #                 'format': 'type=book', 'url': 'https://git.door43.org/Door43-Catalog/shu_obs/archive/master.zip'}, '/tmp/shu_obs')
    # print(res)
    event = {
        'stage': 'dev',
        'stage-variables': {
            'cdn_bucket': 'dev-cdn.door43.org',
            'cdn_url': 'https://dev-cdn.door43.org',
            'from_email': 'admin@door43.org',
            'to_email': 'rich.mahn@unfoldingword.org',
            'log_level': 'debug',
        },
    }
    context = {}
