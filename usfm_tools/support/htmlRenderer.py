...
class HTMLRenderer(AbstractRenderer):
    ...
    def cleanHTML(self, t):
        ...
        c = c.replace(u'~', u'&nbsp;')
        if self.oebFlag:
            c = c.replace(u'%navmarker%', u'OEB')
            c = c.replace(u'%linkToWebsite%',u'<tr><td colspan = "2"><a href="http://openenglishbible.org">OpenEnglishBible.org</a></td></tr>')
        else:
            c = c.replace(u'%navmarker%', u'<div style="font-size:200%;color:green;">✝</div>')
            c = c.replace(u'%linkToWebsite%',u'')
        return c
...
indexPage = header + u"""<h1>Bible</h1>""" + footer
