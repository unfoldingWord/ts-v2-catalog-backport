...
from .libraries.tools.ts_v2_utils import convert_rc_links, make_legacy_date, \
    max_modified_date, get_rc_type, prep_data_upload, date_is_older, max_long_modified_date, \
    get_project_from_manifest, tn_tsv_to_json, index_chunks, build_json_source_from_usx, build_usx
from .libraries.tools.error_reporter import ErrorReporter

class TsV2CatalogBackporter():
    cdn_root_path = 'v2/ts'
    api_version = 'ts.2'
    ...
    def __init__(self, prefix='', cdn_bucket=None, cdn_url=None, log_level=None, 
        ...
        logging.basicConfig()
        logging.getLogger('boto3').setLevel(boto_logging_level)
        logging.getLogger('botocore').setLevel(boto_logging_level)
        ...
        self.reporter = ErrorReporter(reporter='tx_v2_catalog_backport',
                                      table='{}d43-catalog-errors'.format(self.prefix),
                                      to_email=to_email,
        ...
        if not self.cdn_bucket:
            self.cdn_bucket = f'{self.prefix}cdn.door43.org'

        self.cdn_url = cdn_url
        if not self.cdn_url:
            self.cdn_url = f'https://{self.cdn_bucket}'

        self.cdn_handler = S3Handler(self.cdn_bucket)  # pragma: no cover
        self.db_handler = DynamoDBHandler('{}d43-catalog-status'.format(self.prefix))  # pragma: no cover
    ...
    def __set_logging_level(self, level):
        ...
        if level == 'info':
            ...
            logging.getLogger('usfm_tools').setLevel(logging.ERROR)
        else:
    ...
    def run(self):
        ...
        try:
            ...
            raise Exception(EnvironmentError('Bad Request: {}'.format(e))).with_traceback(sys.exc_info()[2])
        finally:
    ...
    def _run(self):
        ...
        try:
            ...
            raise Exception(Exception(e)).with_traceback(sys.exc_info()[2])
    ...
    def __execute(self):
                    ...
                    for format in res['formats']:
                        ...
                        if rid != 'obs':
                            ...
                            if process_id not in self.status['processed']:
                                ...
                                if tn:
                                    self._upload_all(tn)
                                    finished_processes[process_id] = list(tn.keys())
                                    cat_keys = cat_keys + list(tn.keys())
                            else:
                            ...
                            if process_id not in self.status['processed']:
                                ...
                                if tq:
                                    self._upload_all(tq)
                                    finished_processes[process_id] = list(tq.keys())
                                    cat_keys = cat_keys + list(tq.keys())
                            else:
                                cat_keys = cat_keys + self.status['processed'][process_id]
                    ...
                    if 'formats' in project:
                        for format in project['formats']:
                            ...
                            if process_id not in self.status['processed']:
                                ...
                                if tw:
                                    self._upload_all(tw)
                                    finished_processes[process_id] = list(tw.keys())
                                    cat_keys = cat_keys + list(tw.keys())
                            else:
                                cat_keys = cat_keys + self.status['processed'][process_id]
                            ...
                            if rid == 'obs':
                                ...
                                if process_id not in self.status['processed']:
                                    if self._has_resource_changed('obs', lid, rid, format['modified']):
                                        ...
                                        self.logger.debug('Skipping OBS {0}-{1} because it hasn\'t changed'.format(lid, rid))
        ...
        for upload in api_uploads:
            if not upload['key'].startswith('/'):
                key = '{}/{}'.format(self.cdn_root_path, upload['key'])
            else:
    ...
    def _set_status(self):
        ...
        self.db_handler.update_item({'api_version': self.api_version}, self.status)
    ...
    def _has_resource_changed(self, pid, lid, rid, modified_at):
        ...
        if cache_key in self.ts_resource_cache:
            ...
            self.logger.debug(f"Getting resources.json file from {ts_resource_url}")
            ts_resources = self.get_url(ts_resource_url, True)
            if ts_resources:
                self.logger.debug(f"{ts_resource_url} FOUND!")
                self.ts_resource_cache[cache_key] = ts_resources
            else:
                self.logger.debug(f"{ts_resource_url} NOT FOUND!")
    ...
    def _get_url_date_modified(self, url_string):
        try:
            ...
            value = urllib.parse.parse_qs(url.query)['date_modified']
            if len(value) > 0:
                return value[0]
            else:
    ...
    def _has_language_changed(self, pid, lid, modified_at):
        ...
        if cache_key in self.ts_languages_cache:
            ...
            ts_languages = self.get_url('{}}/v2/ts/{}/languages.json'.format(self.cdn_url, pid), True)
            if ts_languages:
    ...
    def _has_project_changed(self, pid, modified_at):
        ...
        if not self.ts_projects_cache:
            self.ts_projects_cache = self.get_url('{}/v2/ts/catalog.json'.format(self.cdn_url), True)
    ...
    def _get_status(self):
        ...
        for s in status_results:
            if s['api_version'] == '3':
                source_status = s
            elif s['api_version'] == self.api_version:
                status = s
    ...
    def _tn_tsv_to_json_file(self, lid, rid, resource, format, temp_dir):
        ...
        for project in resource['projects']:
            ...
            if pid != 'obs':
                try:
                    data = get_url('{}/bible/txt/1/{}/chunks.json'.format(self.cdn_url, pid))
            ...
            # convert
            self.logger.debug(f"Converting {lid}-{rid}-{pid} TSV file: {note_file}...")
            with open(note_file, newline='') as tsvin:
    ...
    def _tn_md_to_json_file(self, lid, rid, resource, format, temp_dir):
        ...
        for project in resource['projects']:
            ...
            for chapter in chapters:
                ...
                verses = sorted(os.listdir(chapter_dir))
                ...
                for cat in categories:
                    ...
                    for word in word_files:
                        ...
                        try:
                            ...
                            self.report_error('Failed to read file {}: {}'.format(word_path, e))
                            raise
            ...
            for project in resource['projects']:
                ...
                if process_id not in self.status['processed']:
                    ...
                    self.cdn_handler.upload_file(upload['path'],
                                                 '{}/{}'.format(self.cdn_root_path, upload['key']))
