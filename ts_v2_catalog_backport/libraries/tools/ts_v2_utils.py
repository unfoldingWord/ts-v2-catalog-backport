...
from .usfm_utils import usfm3_to_usfm2
from .versification import hebrew_to_ufw
...
def usx_to_chunked_json(usx, chunks, lid, pid):
    ...
    for line in usx:
        ...
        if chunk_chapter not in chunks:
            raise Exception('Missing chapter {} in {} {} chunk json'.format(chunk_chapter, lid, pid))
        ...
        try:
            ...
            if reporter:
                reporter.report_error(
                    'Unable to parse verses from chunk {}: {} ({})'.format(chp_num, fr_text, path))
