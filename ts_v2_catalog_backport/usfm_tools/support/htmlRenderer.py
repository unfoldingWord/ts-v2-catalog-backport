...
class HTMLRenderer(AbstractRenderer):
    ...
    def cleanHTML(self, t):
        ...
        c = c.replace('~', '&nbsp;')
        if self.oebFlag:
            c = c.replace('%navmarker%', 'OEB')
            c = c.replace('%linkToWebsite%','<tr><td colspan = "2"><a href="http://openenglishbible.org">OpenEnglishBible.org</a></td></tr>')
        else:
            c = c.replace('%navmarker%', '<div style="font-size:200%;color:green;">✝</div>')
            c = c.replace('%linkToWebsite%','')
        return c
...
indexPage = header + """<h1>Bible</h1>""" + footer
